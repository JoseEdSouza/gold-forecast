...
RAMDOM_SEED = 42
WINDOW_SIZE_MIN = (
    10  # Valor mínimo/fallback; será ajustado com base em lags significativos da PACF
)
...
log.info(
    "Série de retornos: primeiras linhas:\n%s",
...
log.info(
    "Eventos extremos (> %.1f desvios): %d ocorrências", LIMIAR_SIGMA, len(eventos)
)
log.info("Primeiros eventos:\n%s", eventos.head(10).to_string())

plt.figure(figsize=(14, 5))
plt.plot(
    df["timestamp"], df["log_return"], linewidth=0.6, alpha=0.8, label="Log return"
)
plt.scatter(
    eventos["timestamp"],
    eventos["log_return"],
    ...
    s=20,
    zorder=5,
    label=f"Extremos (>{LIMIAR_SIGMA}σ)",
)
...
df_model = (
    df[feature_cols + ["target", "timestamp", "close"]].dropna().reset_index(drop=True)
)
...
log.info(
    "Coef. mais relevantes (top 5 por magnitude):\n%s",
    pd.Series(model_lr.coef_, index=feature_cols).abs().nlargest(5).to_string(),
)
...
axes[0].plot(
    xr, stats.norm.pdf(xr, mu_r, sig_r), color="#E24B4A", lw=1.5, label="Normal teórica"
)
