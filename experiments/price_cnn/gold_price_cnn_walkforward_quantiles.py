...
def walk_forward(df, feature_cols, epochs=40, fold_start=1, fold_end=99):
    ...
    while train_end + EMBARGO < n - 10:
        ...
        sc = lambda X: np.clip(
            xs.transform(X.reshape(-1, nf)).reshape(X.shape), -8, 8).astype(np.float32)
        Xtr, Xva, Xte = sc(Xtr), sc(Xva), sc(Xte)
        ys = RobustScaler().fit(Ytr)
        d = lambda Y: {f"h{h}": ys.transform(Y).astype(np.float32)[:, j]
                       for j, h in enumerate(HORIZONS)}

        tf.keras.backend.clear_session()
        model = build_model(nf)
        model.fit(Xtr, d(Ytr), validation_data=(Xva, d(Yva)),
        ...
        print(f"[fold {fold}] treino ate idx {train_end} | teste {d0} -> {d1} "
              f"| {len(Xte)} amostras")
...
def report(res: pd.DataFrame):
    ...
    for h in HORIZONS:
        ...
        print(f"h={h:>2}d | MAE ${mae:7.2f} | MAPE {mape:5.2f}% | Dir.Acc {dacc:5.1f}% "
    ...
    for ano, g in res.groupby("ano"):
        ...
        print(f"{ano} | n={len(g):>3} | MAE/naive {ratio:.3f} | Dir.Acc {dacc:5.1f}% "
