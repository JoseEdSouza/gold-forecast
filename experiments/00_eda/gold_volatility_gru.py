...
dedolar = pd.read_csv(
    DATA_BASE_PATH / "petrodollar_4_dedollarization.csv", parse_dates=["date"]
)
...
palladium = pd.read_csv(
    DATA_BASE_PATH / "final_palladium_data.csv",
    sep=";",
    encoding="utf-8",
    parse_dates=["timestamp"],
)
platinum = pd.read_csv(
    DATA_BASE_PATH / "final_platinum_data.csv",
    sep=";",
    encoding="utf-8",
    parse_dates=["timestamp"],
)
...
gold["bb_position"] = (gold["close"] - (gold["sma_20"] - 2 * rolling_std)) / (
    4 * rolling_std
)  # adimensional [0,1]
...
gold["rv_20_rank"] = gold["rv_20"].rolling(252).rank(pct=True)

gold["vix_rank"] = gold["vix"].rolling(252).rank(pct=True)
...
cols_to_remove = [
    ...
    "high",
    "low",
    "open",  # em nível — usar só close como âncora temporal
...
X = gold.drop(
    columns=[c for c in to_drop + [TARGET] + cols_to_remove if c in gold.columns]
)
y = gold[TARGET]
...
correlations = (
    corr.corr()["target"].drop("target").sort_values(key=abs, ascending=False)
)
correlations
...
df = pd.DataFrame({"Return": y_train_full.flatten()})
...
for col in externos.columns:
    if col != "timestamp":
        ...
        if first_valid is not None:
            print(col, externos.loc[first_valid, "timestamp"])
...
for fold, (_, val_idx) in enumerate(folds):
    ...
    print(fold + 1, y_va.mean(), y_va.std(), y_va.min(), y_va.max())
...
for fold, (_, val_idx) in enumerate(folds):
    print(fold + 1, seq_ts.iloc[val_idx[0]], seq_ts.iloc[val_idx[-1]])
